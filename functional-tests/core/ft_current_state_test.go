...
var _ = Describe("Working with the current state of Hoverfly via the API", func() {
	...
	It("Should be able to put, get, patch and delete state", func() {
		...
		Expect(ioutil.ReadAll(res.Body)).To(MatchJSON(`{"foo":"bar","cheese":"ham"}`))
		...
		// PATCH
		req = sling.New().Patch(stateURL).BodyJSON(map[string]string{"foo": "patched"})
		res = functional_tests.DoRequest(req)
		Expect(res.StatusCode).To(Equal(200))
		Expect(ioutil.ReadAll(res.Body)).To(MatchJSON(`{"cheese":"ham","foo":"patched"}`))
